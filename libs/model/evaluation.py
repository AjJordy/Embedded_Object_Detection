...
def filter_batch(y_pred,config):
    ...
    for j in range(config.BATCH_SIZE):
        ...
        filtered_bbox, filtered_score, filtered_class = filter_prediction(det_boxes[j], det_probs[j], det_class[j], config)
...
def compute_statistics_for_thresholding(all_boxes, all_classes, all_scores, all_gts, config):
    ...
    all_tps = [] # True positive
    all_fps = [] # False positive

    all_fns = [] # False negative
